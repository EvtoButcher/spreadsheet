...
namespace ASTImpl {
    ...
    public:
        ...
        virtual double Evaluate(GetCellPtr cell_ptr) const = 0;
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(GetCellPtr cell_ptr) const override {
                double result;
                double lhs_arg = lhs_->Evaluate(cell_ptr);
                double rhs_arg = rhs_->Evaluate(cell_ptr);
                ...
                case Multiply:
                    ...
                    break;
                case Divide:
                    if (!std::isfinite(lhs_arg / rhs_arg))
        ...
        public:
            ...
            double Evaluate(GetCellPtr cell_ptr) const override {
                double real_operand = operand_->Evaluate(cell_ptr);
                switch (type_)
                {
                case UnaryPlus:
                    return real_operand;
        ...
        public:
            ...
            double Evaluate(GetCellPtr get_cell_ptr) const override {
                
                if (!cell_pos_->IsValid()) {
                    throw FormulaError(FormulaError::Category::Ref);
                }
                auto cell_ptr = get_cell_ptr(*cell_pos_);
                if (!cell_ptr) {
                    return 0;
                }
                ...
                if (std::holds_alternative<double>(value)) {
                    ...
                    std::string non_space_value_str = value_str.substr(value_str.find_first_not_of(' '), 
                        value_str.find_last_not_of(' ') + 1 - value_str.find_first_not_of(' '));
                    double d_value;
                    try
                    {
                        d_value = std::stod(non_space_value_str);
                    }
...
double FormulaAST::Execute(GetCellPtr cell_ptr) const {
    return root_expr_->Evaluate(cell_ptr);
}
